...
infile = open("minimap2_hifi.Ppr.italy.phased.fasta.paf", "r")
	...
	for i in range(0, len(reads_coord[read])):
		for j in range(1, len(reads_coord[read])):
			...
			if reads_coord[read][i][0] is not reads_coord[read][j][0]:
				if reads_coord[read][i][1] < reads_coord[read][j][1] and reads_coord[read][i][2] < reads_coord[read][j][2] and reads_coord[read][i][2] <= reads_coord[read][j][1]:
					#print(reads_coord[read][i][0],reads_coord[read][j][0])
					if reads_coord[read][i][0] in haplotigs.keys() and reads_coord[read][j][0] in haplotigs[reads_coord[read][i][0]].keys():
